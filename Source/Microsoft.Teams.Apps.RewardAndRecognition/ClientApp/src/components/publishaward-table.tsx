...
const PublishAwardTable: React.FunctionComponent<IPublishAwardTableProps> = props => {
    ...
    let awardsTableRows = props.publishData.map((value: any, index) => {
        let totalLabels = value.NominatedToName.trimEnd(',').split(',');
        let labels = totalLabels.map((name, index) => {
            if (index <= 1) {
                return (
                    <Label className="label-color label-margin" onClick={() => onItemClick(value.NominatedToName, true)}  circular content={name} />
                )
            }
            else if (index === 2) {
                return (
                    <Label onClick={() => onItemClick(value.NominatedToName, true)} className="label-color" circular content={`+${totalLabels.length - 2}`} />
                )
            }
        }
        );
